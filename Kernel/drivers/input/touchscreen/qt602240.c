...
#if defined(CONFIG_HAWK_VER_B1_REAL_ADDED_FEATURE) //NAGSM_Android_HQ_KERNEL_CLEE_20100914 : Key LED Setup in Hawk 0.1
...
void touch_led_set_voltage(int val)
{
	switch( val )
	{
	case 1 : 	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_2p400); break;
	case 2 :	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_2p500); break;
	case 3 : 	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_2p600); break;
	case 4 :	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_2p700); break;
	case 5 : 	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_2p800); break;
	case 6 :	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_2p900); break;
	case 7 : 	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_3p000); break;
	case 8 :	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_3p100); break;
	case 9 : 	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_3p200); break;
	case 10 :	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_3p300); break;	
	case 255 :	Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_3p300); break;
...
void TSP_forced_release_forOKkey(void)
{
	...
	for ( i=0; i<MAX_USING_FINGER_NUM; i++ )
	{
		...
		input_report_abs(qt602240->input_dev, ABS_MT_POSITION_X, fingerInfo[i].x);
		input_report_abs(qt602240->input_dev, ABS_MT_POSITION_Y, fingerInfo[i].y);
		input_report_abs(qt602240->input_dev, ABS_MT_TOUCH_MAJOR, fingerInfo[i].pressure);	// 0이?Release, 팥?Press 상태(Down or Move)
		input_report_abs(qt602240->input_dev, ABS_MT_WIDTH_MAJOR, fingerInfo[i].size_id);	// (ID<<8) | Size
		input_mt_sync(qt602240->input_dev);
...
//					cal_check_flag = 2u;
				...
				if(quantum_msg[0] == 0xc)
				{
					...
					if(quantum_msg[1] == 0x80)
					{
						...
						{
							printk("[TSP Key] Back key pressed.\n");
							touch_key_back_pressed = 1;
							input_report_key(qt602240->input_dev,28, 1);
							
						}
						else if ( quantum_msg[2] == 0x2) //Menu
						{
							printk("[TSP Key] Menu key pressed.\n");
							touch_key_menu_pressed = 1;
							input_report_key(qt602240->input_dev,158, 1);
							
						}
						else if ( quantum_msg[2] == 0x4) // Search
						{
							printk("[TSP Key] Search key pressed.\n");
							touch_key_search_pressed = 1;
							input_report_key(qt602240->input_dev,107, 1);							
						}
						else
						{
					...
					else
					{
						...
						if ( touch_key_back_pressed || touch_key_menu_pressed || touch_key_search_pressed )
						{
							...
							{
								printk("[TSP Key] Back key released.\n");								
								input_report_key(qt602240->input_dev,28, 0);
								touch_key_back_pressed = 0;								
							}
							if ( touch_key_menu_pressed )	//Menu
							{
								printk("[TSP Key] Menu key released.\n");
								input_report_key(qt602240->input_dev,158, 0);
								touch_key_menu_pressed = 0;								
							}
							if ( touch_key_search_pressed )	// Search
							{
								printk("[TSP Key] Search key released.\n");
								input_report_key(qt602240->input_dev,107, 0);								
...
#if defined(CONFIG_HAWK_VER_B1_REAL_ADDED_FEATURE) //NAGSM_Android_HQ_KERNEL_CLEE_20100908 : Setup Hawk Real Board Rev 0.1
int qt602240_keycode[] = {50,158,28,0,42,58};
#endif
...
//	DEBUG;
	...
	#if defined(CONFIG_HAWK_VER_B1_REAL_ADDED_FEATURE) //NAGSM_Android_HQ_KERNEL_CLEE_20100908 : Setup Hawk Real Board Rev 0.1
	set_bit(28,  qt602240->input_dev->keybit);
	set_bit(158, qt602240->input_dev->keybit);
	set_bit(107, qt602240->input_dev->keybit);
