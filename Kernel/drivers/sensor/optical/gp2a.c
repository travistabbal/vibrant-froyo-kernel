...
static unsigned char vout_val = 1;
...
irqreturn_t gp2a_irq_handler(int irq, void *dev_id)
{
	...
	char value_VOUT;
	value_VOUT = gpio_get_value(GPIO_PS_VOUT);
...
//	disable_irq(IRQ_GP2A_INT);
	...
	printk(KERN_INFO "[gp2a_irq_handler] vout = %d \n",vout);
...
#if defined (CONFIG_S5PC110_T959_BOARD) || defined (CONFIG_S5PC110_VIBRANTPLUS_BOARD)
	if(!proximity_enable)
	{	
...
static int proximity_onoff(u8 onoff)
{
	...
	if(onoff)
	{
		...
		s3c_gpio_setpull(GPIO_PS_VOUT, S3C_GPIO_PULL_NONE);
#if defined (CONFIG_S5PC110_VIBRANTPLUS_BOARD)
		set_irq_type(IRQ_GP2A_INT, IRQ_TYPE_EDGE_FALLING /* IRQ_TYPE_EDGE_BOTH */ );
#else
...
#if defined (CONFIG_S5PC110_VIBRANTPLUS_BOARD)
	printk(KERN_INFO "[PROXIMITY] vout_val = %d \n",vout_val);
	put_user(vout_val, buf);
	return 1;
#else
	char value;
	//printk("[%s] \n",__func__);
